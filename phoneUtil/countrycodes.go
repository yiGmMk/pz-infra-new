...
var countryCodeInitializer = &sync.Once{}
var countryCodeMap map[string]string

func getCountryCodeMap() map[string]string {
	countryCodeInitializer.Do(func() {
		result := make(map[string]string, 0)
		for i := 0; i < len(countryCodes); i += 2 {
			country := countryCodes[i]
			phoneCode := countryCodes[i+1]
			result[country] = phoneCode
		}
		countryCodeMap = result
	})
...
var countryCodes = []string{
	...
	"DJ", "253", //Djibouti
	"DM", "1767", //Dominica
	"DO", "1809", //Dominican Republic, 1809, 1829, 1849
	"TL", "670", //East Timor
	"EC", "593", //Ecuador
	"EG", "20", //Egypt
	"SV", "503", //El Salvador
