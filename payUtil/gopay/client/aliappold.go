...
func InitAliAppOldClient(c *AliAppOldClient) {
	defaultAliAppOldClient = c
}

// DefaultAliAppClient 得到默认支付宝app客户端
func DefaultAliAppOldClient() *AliAppOldClient {
	return defaultAliAppOldClient
}

func (aa *AliAppOldClient) Pay(charge *common.Charge) (string, error) {
...
// GenSign 产生签名
func (aa *AliAppOldClient) GenSign(m map[string]string) (string, error) {
	delete(m, "sign_type")
	delete(m, "sign")
	var data []string
	for k, v := range m {
...
// CheckSign 检测签名
func (aa *AliAppOldClient) CheckSign(data string, sign string) error {
